...
export const expanders: Controller[] = [];
...
try {
    ...
    appConfig.gpio.expanders.forEach((expander, i) => {
        const controllerCreator = controllers.get(expander.type);
        if (!controllerCreator) return;
        expanders.push(controllerCreator(expander.environment));
